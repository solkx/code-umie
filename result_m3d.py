...
def entity_prc(pre_entity, pre_link, gold_entity, gold_link):
    ...
    b_e, t_e, total_ent_r = error_ent(list(set(pre_entity) - set(gold_entity)), list(set(gold_entity) - set(pre_entity)))
    ...
    return p, r, c, ceaf_p, ceaf_r, ceaf_c, muc_p, muc_r, muc_c, b3_c_p, b3_c_r, b3_p, b3_r, b_e, t_e, total_ent_r
...
def decode_link_rel(result, gold_link, gold_rel, doc, video_id, is_loose, is_big_id, is_drop_id, lang):
    ...
    # print(gold_entity)
    # print("+"*100)
    p, r, c, ceaf_p, ceaf_r, ceaf_c, muc_p, muc_r, muc_c, b3_c_p, b3_c_r, b3_p, b3_r, b_e, t_e, total_ent_r = entity_prc(pre_entity_list, link_list, gold_entity, gold_link)
    rel_p, rel_r, rel_c = rel_prc(pre_relType2link, gold_relType2link)
    # ground_pred, ground_gold = grounding_encode_new_ap(grounding_result, video_id)
    ground_pred, ground_gold = [], []
    ground_p, ground_r, ground_c = grounding_encode_new(grounding_dic, video_id, lang)
    return p, r, c, ceaf_p, ceaf_r, ceaf_c, muc_p, muc_r, muc_c, b3_c_p, b3_c_r, b3_p, b3_r, rel_p, rel_r, rel_c, ground_pred, ground_gold, ground_p, ground_r, ground_c, b_e, t_e, total_ent_r
...
if __name__ == "__main__":
    ...
    mode_list = [["result","all-llama", "en", 0.0002], ["result","all-baichuan", "zh", 0.0008]]
    tabel_list = []
    for root, mode, lang, lr in mode_list:
        ...
        result_list = sorted(result_list.items(), key=lambda x:x[-1][-5], reverse=True)
        print([f"{item[0]}-{round(item[-1][-5], 4)}" for item in result_list])
        result_list = result_list[:3]
        if result_list:
            ent_precious_list = [item[-1][0] for item in result_list]
            ent_precious = res(ent_precious_list)
            ent_recall_list = [item[-1][1] for item in result_list]
            ent_recall = res(ent_recall_list)
            ent_f1_list = [item[-1][2] for item in result_list]
            ent_f1 = res(ent_f1_list)
            muc_precious_list = [item[-1][3] for item in result_list]
            muc_precious = res(muc_precious_list)
            muc_recall_list = [item[-1][4] for item in result_list]
            muc_recall = res(muc_recall_list)
            muc_f1_list = [item[-1][5] for item in result_list]
            muc_f1 = res(muc_f1_list)
            ceaf_precious_list = [item[-1][6] for item in result_list]
            ceaf_precious = res(ceaf_precious_list)
            ceaf_recall_list = [item[-1][7] for item in result_list]
            ceaf_recall = res(ceaf_recall_list)
            ceaf_f1_list = [item[-1][8] for item in result_list]
            ceaf_f1 = res(ceaf_f1_list)
            b3_precious_list = [item[-1][9] for item in result_list]
            b3_precious = res(b3_precious_list)
            b3_recall_list = [item[-1][10] for item in result_list]
            b3_recall = res(b3_recall_list)
            b3_f1_list = [item[-1][11] for item in result_list]
            b3_f1 = res(b3_f1_list)
            link_precious_list = [item[-1][12] for item in result_list]
            link_precious = res(link_precious_list)
            link_recall_list = [item[-1][13] for item in result_list]
            link_recall = res(link_recall_list)
            link_f1_list = [item[-1][14] for item in result_list]
            link_f1 = res(link_f1_list)
            rel_precious_list = [item[-1][15] for item in result_list]
            rel_precious = res(rel_precious_list)
            rel_recall_list = [item[-1][16] for item in result_list]
            rel_recall = res(rel_recall_list)
            rel_f1_list = [item[-1][17] for item in result_list]
            rel_f1 = res(rel_f1_list)
            ground_precious_list = [item[-1][18] for item in result_list]
            ground_precious = res(ground_precious_list)
            ground_recall_list = [item[-1][19] for item in result_list]
            ground_recall = res(ground_recall_list)
            ground_f1_list = [item[-1][20] for item in result_list]
            ground_f1 = res(ground_f1_list)
            ground_map_list = [item[-1][21] for item in result_list]
            ground_map = res(ground_map_list)
            avg_precious_list = [item[-1][22] for item in result_list]
            avg_precious = res(avg_precious_list)
            avg_recall_list = [item[-1][23] for item in result_list]
            avg_recall = res(avg_recall_list)
            avg_f1_list = [item[-1][24] for item in result_list]
            avg_f1 = res(avg_f1_list)
            avg_ent_b_e_list = [item[-1][25] for item in result_list]
            avg_ent_b_e = res(avg_ent_b_e_list)
            avg_ent_t_e_list = [item[-1][26] for item in result_list]
            avg_ent_t_e = res(avg_ent_t_e_list)
            avg_avg_f1_list = [item[-1][27] for item in result_list]
            avg_avg_f1 = res(avg_avg_f1_list)
            # print(result_list)
            ...
            table.add_row(["Entity"] + [f"{ent_precious[0]}({ent_precious[-1]})", f"{ent_recall[0]}({ent_recall[-1]})", f"{ent_f1[0]}({ent_f1[-1]})"])
            table.add_row(["Link(MUC)"] + [f"{muc_precious[0]}({muc_precious[-1]})", f"{muc_recall[0]}({muc_recall[-1]})", f"{muc_f1[0]}({muc_f1[-1]})"])
            table.add_row(["Link(CEAF)"] + [f"{ceaf_precious[0]}({ceaf_precious[-1]})", f"{ceaf_recall[0]}({ceaf_recall[-1]})", f"{ceaf_f1[0]}({ceaf_f1[-1]})"])
            table.add_row(["Link(B3)"] + [f"{b3_precious[0]}({b3_precious[-1]})", f"{b3_recall[0]}({b3_recall[-1]})", f"{b3_f1[0]}({b3_f1[-1]})"])
            table.add_row(["Link(Avg.)"] + [f"{link_precious[0]}({link_precious[-1]})", f"{link_recall[0]}({link_recall[-1]})", f"{link_f1[0]}({link_f1[-1]})"])
            table.add_row(["Relation"] + [f"{rel_precious[0]}({rel_precious[-1]})", f"{rel_recall[0]}({rel_recall[-1]})", f"{rel_f1[0]}({rel_f1[-1]})"])
            table.add_row(["Grounding"] + [f"{ground_precious[0]}({ground_precious[-1]})", f"{ground_recall[0]}({ground_recall[-1]})", f"{ground_f1[0]}({ground_f1[-1]})"])
            table.add_row(["Avg."] + [f"{avg_precious[0]}({avg_precious[-1]})", f"{avg_recall[0]}({avg_recall[-1]})", f"{avg_f1[0]}({avg_f1[-1]})"])
            table.add_row(["Ent"] + [f"{avg_ent_b_e[0]}", f"{avg_ent_t_e[0]}", f""])
    ...
    for table in tabel_list:
        for line in str(table).split("\n"):
            if "|" not in line or "Avg.-f1" in line:
                continue
            hang += 1
            lie = 0
    ...
    wb.save("tabel.xlsx")
            ...
            # with open(f"./error_{mode}.json", "w", encoding="utf-8") as f:
            #     f.write(json.dumps(error, ensure_ascii=False))
            # with open(f"./rel_prc_{mode}.json", "w", encoding="utf-8") as f:
            #     f.write(json.dumps(rel_prc_list, ensure_ascii=False))
            # print(sum([item[-1] for item in rel_prc_list]))