...
def load_video_base_img(video_path, n_frms=MAX_INT, height=-1, width=-1, sampling="uniform", return_msg = False):
    # decord.bridge.set_bridge("torch")
    ...
    tensor_frms = []
    for imgName in os.listdir(video_path):
        if ".txt" in imgName:
            continue
        image = cv2.imread(f"{video_path}/{imgName}")
        image_rgb = cv2.resize(image, (width, height), interpolation=cv2.INTER_LINEAR)
        tensor_frms.append(torch.from_numpy(image_rgb))
    if tensor_frms:
        tensor_frms = torch.stack(tensor_frms, dim=0)
        frms = tensor_frms.permute(3, 0, 1, 2).float()  # (C, T, H, W) 3 31 224 224
        if not return_msg:
            return frms
        ...
        return torch.zeros((3,1,224,224))
